...
$t="SELECT * FROM `patient` WHERE blood='$gender'";
...
if($r)
{
	...
	while($w > 0)
	{
						...
						{  
							...
							$query = "INSERT INTO `match`(`emailp`, `emaild`,`blood`,`tissuep`,`tissued`) VALUES ('$m','$email','$gender','$b','$user')";
					  		...
					  		{        
					    		   ...
					    		   //delet from patient
									$f=" DELETE FROM  `patient` WHERE email = '$m'";
			...
			if($c1==0)
			{
				$query = "INSERT INTO `donator`(`email`,`tissue`,`blood`,`birth`) VALUES ('$email','$user','$gender','$bday')";
			    if (mysqli_query($con,$query)) 
			    {        
			  	   echo "<h1>Data has been inserted into database</h1> ";
			  	   header("Location:dnpage.html");
			    }
			    else 
			    {
		  		    echo "Error: " . mysqli_error($con);
