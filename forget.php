...
// checks if the username is in use
	$check = mysqli_query($con,"SELECT * FROM doc WHERE email = '$email'")or die(mysql_error());
...
// if no errors then carry on
	if (($error == "") && ($error2 == "" )) {
		...
		$query = mysqli_query($con,"SELECT * FROM doc WHERE email = '$email' ")or die (mysql_error());
 
//create a new random password
 
		$password = uniqid(random_string());
		//update database
		$sql = mysqli_query($con,"UPDATE doc SET pass='$password' WHERE email = '$email'")or die (mysql_error());
		
 
//send email
		$to = $email;
		$subject = "Reset doctor's account password";
              ...
              <div class="col-md-6 col-md-offset-3"><br><br><br><br><br>
					<form action="<?php $_SERVER['PHP_SELF']?>" method="post" autocomplete="on">
					    <?php if ($rsent == true){
					           ...
					           echo"<a href=\"changePass.php?email=$to\">Click Here To Change Your Password</a>";

							 }
							else {?>
							<p>Please enter your e-mail address. You will receive a new password via e-mail</p><br>
							<p>Email Address: <input type="text" name="remail" size="50" maxlength="255">
							<br><br>
						    <input type="submit" name="submit" value="Get New Password"></p><br>
